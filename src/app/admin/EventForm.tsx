...
const schema = z.object({
  ...
  // Logique conditionnelle gérée par raffinement plus bas
  isPayant: z.boolean(),
  ...
  if (data.isPayant && (!data.prix || data.prix <= 0)) {
    return false; // Échoue si payant mais pas de prix
  }
  return true;
}, {
  ...
  if (!data.isIllimite && (!data.placesTotal || data.placesTotal <= 0)) {
    return false; // Échoue si limité mais pas de places spécifiées
  }
  return true;
}, {
...
interface EventFormProps {
  initialData?: any; // Pour le mode modification
...
export function EventForm({ initialData, onSubmit }: EventFormProps) {
  ...
  // Observation des toggles en temps réel
  ...
  const handleFormSubmit = async (values: EventFormValues) => {
    setLoading(true);
    try {
      // Nettoyage des données avant envoi à Firestore selon les toggles cochés
      const finalData = {
        ...values,
  ...
  return (
    ...
    >
      ...
      {/* SECTION 2 : PLANIFICATION & LOGISTIQUE FLUIDE */}
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        ...
        {/* LOGISTIQUE DES PLACES (ILLIMITÉ OU SPÉCIFIÉ) */}
        <div className="space-y-5">
          <h3 className="flex items-center gap-2 font-display text-sm font-black uppercase tracking-wider text-slate-400">
            <Users size={16} /> Capacité d&apos;accueil
          </h3>

          {/* CHECKBOX DESIGN LOGIQUE ILLIMITÉE */}
          ...
          {/* LE CHAMP DES PLACES APPARAÎT S'IL N'EST PAS ILLIMITÉ */}
          {!isIllimite && (
      ...
      {/* SECTION 3 : STRATÉGIE DE REVENU (TOGGLE PREMIUM VISUEL) */}
      <div className="space-y-5">
        <h3 className="flex items-center gap-2 font-display text-sm font-black uppercase tracking-wider text-slate-400">
          <Coins size={16} /> Configuration d&apos;accès financier
        </h3>

        {/* TOGGLE PREMIUM PAYANT/GRATUIT */}
        ...
        {/* LE CHAMP MONTANT S'AFFICHE EXCLUSIVEMENT SI LE TOGGLE EST SUR PAYANT */}
        {isPayant && (
      ...
      {/* BOUTON DE SOUMISSION STYLE VERRE FIXÉ AU DESIGN */}
